...
export class CryptoService {
  ...
  async encryptAsync(data: string, destiny = 'local'): Promise<string> {
    ...
    if (data.startsWith(this.ENCRYPTION_PREFIX)) {
      const dataToEncrypt = data.substring(this.ENCRYPTION_PREFIX.length);
    ...
    const keyBytes = Uint8Array.from(atob(this.secretKey), c => c.charCodeAt(0));// NOSONAR
  ...
  async decryptAsync(encryptedData: string): Promise<string> {
    try {
      ...
      throw new Error('Error al desencriptar: clave incorrecta o datos corruptos', { cause: error });
    }
  }
  ...
  private validateKey(secretKey: string): void {
    ...
    try {
      if (this.isKeyValidated) {
        ...
        const keyBytes = Uint8Array.from(atob(secretKey), c => c.charCodeAt(0));// NOSONAR
      ...
      throw new Error('La clave debe ser un base64 válido', { cause: error });
    }
  }
  ...
  private uint8ArrayToBase64(bytes: Uint8Array): string {
    ...
    for (let i = 0; i < bytes.length; i += chunkSize) {
      ...
      binary += String.fromCharCode.apply(null, Array.from(chunk)); // NOSONAR
    }
